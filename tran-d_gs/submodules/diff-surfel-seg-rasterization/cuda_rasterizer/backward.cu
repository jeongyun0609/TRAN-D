...
renderCUDA(
	...
	const float* __restrict__ dL_dpixels_obj,
	...
	float* __restrict__ dL_dsegs,
	float* __restrict__ dL_dobj
	)
{
	...
	__shared__ float collected_segs[C * BLOCK_SIZE];
	__shared__ float collected_objs[O * BLOCK_SIZE];
	...
	float accum_rec[C] = { 0 };
	float accum_rec_seg[C] = { 0 };
	float accum_rec_obj[O] = { 0 };
	float dL_dpixel[C];
	float dL_dpixel_seg[C];
	...
	if (inside){
		...
		for (int i = 0; i < O; i++)
			dL_dpixel_obj[i] = dL_dpixels_obj[i * H * W + pix_id];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < C; i++)
			{
				...
				collected_segs[i * BLOCK_SIZE + block.thread_rank()] = segs[coll_id * C + i];
			}
			for (int i = 0; i < O; i++)
				collected_objs[i * BLOCK_SIZE + block.thread_rank()] = obj[coll_id * O + i];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float dchannel_dseg = alpha * T;
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				const float c_seg = collected_segs[ch * BLOCK_SIZE + j];  //i
				// Update last color (to be used in the next iteration)
				accum_rec[ch] = last_alpha * last_color[ch] + (1.f - last_alpha) * accum_rec[ch];
				last_color[ch] = c;

				accum_rec_seg[ch] = last_alpha * last_seg[ch] + (1.f - last_alpha) * accum_rec_seg[ch];  // i+1 to N
				last_seg[ch] = c_seg; //i+1

				const float dL_dchannel = dL_dpixel[ch];
				const float dL_dchannel_seg = dL_dpixel_seg[ch];
				dL_dalpha += (c - accum_rec[ch]) * dL_dchannel;
				dL_dalpha += (c_seg - accum_rec_seg[ch]) * dL_dchannel_seg;
				// Update the gradients w.r.t. color of the Gaussian. 
				// Atomic, since this pixel is just one of potentially
				// many that were affected by this Gaussian.
				atomicAdd(&(dL_dcolors[global_id * C + ch]), dchannel_dcolor * dL_dchannel);
				atomicAdd(&(dL_dsegs[global_id * C + ch]), dchannel_dseg * dL_dchannel_seg);
			}

			for (int ch = 0; ch < O; ch++)
			{
				const float o = collected_objs[ch * BLOCK_SIZE + j];
				accum_rec_obj[ch] = last_alpha * last_obj[ch] + (1.f - last_alpha) * accum_rec_obj[ch];
				last_obj[ch] = o;

				const float dL_dchannel_obj = dL_dpixel_obj[ch];
				dL_dalpha += (o - accum_rec_obj[ch]) * dL_dchannel_obj;

				atomicAdd(&(dL_dobj[global_id * O + ch]), dchannel_dcolor * dL_dchannel_obj);
			}
			...
			for (int i = 0; i < C; i++){
				...
				bg_dot_dpixel += bg_color[i] * dL_dpixel_seg[i];
			}
	...
	glm::vec4* dL_drots)
{
	...
	if (shs_seg)
		computeColorFromSH(idx, D, M, (glm::vec3*)means3D, *campos, shs_seg, clamped, (glm::vec3*)dL_dseg, (glm::vec3*)dL_dmean3Ds, (glm::vec3*)dL_dsh_seg);
...
void BACKWARD::render(
	...
	float* dL_dobj)
{
	renderCUDA<NUM_CHANNELS, NUM_OBJ> << <grid, block >> >(
		ranges,
		point_list,
		W, H,
		focal_x, focal_y,
