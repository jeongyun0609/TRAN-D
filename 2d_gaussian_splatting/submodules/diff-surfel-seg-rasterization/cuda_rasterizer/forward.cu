	...
	bool prefiltered)
{
	...
	if (colors_precomp == nullptr) {
		...
		glm::vec3 result2 = computeColorFromSH(idx, D, M, (glm::vec3*)orig_points, *cam_pos, shs_seg, clamped);
		seg[idx * C + 0] = result2.x;
		seg[idx * C + 1] = result2.y;
		seg[idx * C + 2] = result2.z;
	}
...
renderCUDA(
	...
	const float* __restrict__ features_seg, 
	const float* __restrict__ features_obj, 
	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < CHANNELS; ch++)
			{
				...
				C_seg[ch] += features_seg[collected_id[j] * CHANNELS + ch] * w;
			
			}
			for (int ch = 0; ch < OBJECTS; ch++)
			{
				O[ch] += features_obj[collected_id[j] * OBJECTS + ch]* w;
			}
	...
	if (inside)
	{
		...
		for (int ch = 0; ch < CHANNELS; ch++){
			...
			out_seg[ch * H * W + pix_id] = C_seg[ch] + T * bg_color[ch];
		}
		for (int ch = 0; ch < OBJECTS; ch++){
			out_obj[ch * H * W + pix_id] = O[ch];}
...
void FORWARD::preprocess(int P, int D, int M,
	...
	bool prefiltered)
{
	preprocessCUDA<NUM_CHANNELS, NUM_OBJ> << <(P + 255) / 256, 256 >> > (
		P, D, M,
		means3D,
		scales,
		scale_modifier,
