...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, time = 0, load_iteration=None, shuffle=True, resolution_scales=[1.0], only_objects=False, no_sim = False, views = 6):
        ...
        if self.loaded_iter and shuffle and no_sim:
            self.gaussians.load_ply_training(os.path.join(self.model_path,
                                                "point_cloud",
                                                "iteration_" + str(self.loaded_iter),
                                                "point_cloud.ply"),  scene_info.train_cameras)
            ...
            visible_index = scene_info.train_cameras[0].visible_obj_index
            self.gaussians.load_ply_remove_obj(os.path.join(self.model_path,
                                                "point_cloud",
                                                "iteration_" + str(self.loaded_iter),
                                                "point_cloud.ply"), missing_indices)




        elif self.loaded_iter and shuffle:
