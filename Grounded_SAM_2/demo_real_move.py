...
frame_names_ = [
    p for p in os.listdir(os.path.join(f"data/{seq_index}", f"train_{opt.time:d}/camera/rgb"))
    if os.path.splitext(p)[-1] in [".JPG", ".jpeg", ".JPG", ".JPEG"]
]
frame_names_.sort(key=lambda p: int(os.path.splitext(p)[0]))
frame_names = []
image_name = f"00001"
init_img_path = os.path.join(f"data/{seq_index}", f"train_0/camera/rgb/00001.JPG")
frame_names.append(init_img_path)
for j in range(len(frame_names_)):
    frame_names.append(os.path.join(f"data/{seq_index}", f"train_{opt.time:d}/camera/rgb", frame_names_[j]) )
...
for k in range(1, len(video_segments)):
    segments = video_segments[k]
...
dynamic_cam_image_folder = os.path.join(f"data/{seq_index}",f"train_{opt.time:d}/camera/rgb")
dynamic_cam_mask_folder = os.path.join(f"data/{seq_index}",f"train_{opt.time:d}/camera/mask_")
dynamic_cam_binary_mask_folder = os.path.join(f"data/{seq_index}",f"train_{opt.time:d}/camera/binary_mask")
dynamic_cam_class_folder = os.path.join(f"data/{seq_index}",f"train_{opt.time:d}/camera/class")
dynamic_cam_depth = os.path.join(f"data/{seq_index}",f"train_{opt.time:d}/camera/depth")
pose_folder = os.path.join(f"data/{seq_index}",f"train_{opt.time:d}/pose")
dynamic_image_files = [f for f in sorted(os.listdir(dynamic_cam_mask_folder)) if f.endswith(".png")]


image_file = dynamic_image_files[0]
idx = 0
image_name = image_file.split('.')[0]
pose_idx = idx+1
image_path = os.path.join(dynamic_cam_mask_folder,image_name+".png") 
original_image_path = os.path.join(dynamic_cam_image_folder,f"{pose_idx:05d}"+".JPG") 
